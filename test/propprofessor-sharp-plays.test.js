...
describe('sharp play target book helpers', () => {
  ...
  it('summarizes strict empty-state diagnostics and counts target-book movement failures', () => {
    ...
    assert.deepEqual(summary.classificationSummary.verdictCounts, { Pass: 4 });
    ...
    assert.equal(summary.topNearMisses.length, 3);
    assert.deepEqual(summary.topNearMisses.map((row) => row.pick), [
      'Chicago Bulls',
      'Minnesota Timberwolves',
      'Los Angeles Dodgers'
    ]);
    assert.deepEqual(summary.topNearMisses[0].passReasons, ['consensus_book_count_below_2']);
  });
  ...
  it('keeps existing filtered-row behavior unchanged when diagnostics are added', () => {
    ...
    assert.deepEqual(summary.classificationSummary.verdictCounts, { 'Bet candidate': 1, Pass: 1 });
  });
