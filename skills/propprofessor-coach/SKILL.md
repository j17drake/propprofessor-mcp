...
- **`find_best_price` game param requires full matchup.** Pass `game="PlayerA vs PlayerB"` (or "Away vs Home" order). Passing just one name like `game="Trungelliti"` only returns rows where a comp book uses that single-name key — it'll silently skip the NoVigApp row even when NoVigApp has the match, because NoVigApp's key uses the full matchup. Verified 2026-06-21: Trungelliti ML on NoVigApp at -138 was missed when queried as `game="Trungelliti"`, found when queried as `game="Hussey vs Trungelliti"`. If the user only gives one player name, do a quick `screen_ranked(league=..., participants=[Name])` first to discover the matchup string, then call `find_best_price` with it.
- **American odds direction: less juice = LESS favored.** When a favorite's line moves from -170 → -138, the market thinks they're _less_ likely to win now (≈63% → ≈58%), not more. "Supportive" movement for the favorite means _shorter_ (more juice), e.g. -170 → -200. Same in reverse for dogs: +144 → +125 means the dog got _less_ payoff, i.e. the market thinks they're _more_ likely to win. Always read the direction through implied probability, not the absolute number. Cross-check `recentSharpMoveDirection` / `movementLabel` / `clvProxyPct` from the screen row — if those say "adverse", trust them over your gut read of the line history.
