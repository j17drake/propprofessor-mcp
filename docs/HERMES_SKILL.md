...
**ask**
Parse a natural language betting query into structured components (league, book, market, side, line, player) and suggest the best tool to call. Works as a query router — agents call this first, then call the suggested tool immediately. Routes to the right tool: book queries → quick_screen, player queries → player_context, validation queries ("should I bet X?") → validate_play guidance, general → recommended_bets. No data is fetched by this tool itself — it's a pure parser + router.
