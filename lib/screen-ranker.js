...
function isOppositeSide(row) {
  ...
  const currentSelection = String(row.selection || '').toLowerCase().trim();
  if (!currentSelection) return false;
  // Check if this matches selection2 (opposite of the primary participant).
  const sel2 = String(row.selection2 || '').toLowerCase().trim();
  ...
  if (selections && typeof selections === 'object') {
    for (const key of Object.keys(selections)) {
      ...
      if (sel && typeof sel === 'object') {
        const nestedSel2 = String(sel.selection2 || '').toLowerCase().trim();
...
function rankScreenRows(
  ...
  const partitioned = filterPipelineResult.reduce(
    (acc, item) => {
      ...
      if (item.row?.focusBookMissing && requirePreferredBook) acc.fallbackRows.push(item);
