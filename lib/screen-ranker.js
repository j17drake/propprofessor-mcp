...
function rankScreenRows(
  ...
  // Attach coverage gaps as a non-enumerable property so the caller can see
  // which matches the focus book didn't price without breaking array semantics
  // (callers that do `result.length`, `result.map`, JSON.stringify all still
  // work as expected). Use Object.defineProperty so it doesn't show up in
  // Array.prototype methods or get serialized by JSON.stringify.
 ...
 * nested (selections.null.odds) shapes. Used to detect which books have
 * posted a price for a match when computing coverage gaps.
 * @param {Object} row - Row data.
...
function rankLeagueScreenRows(
  ...
  // Preserve coverageGaps from the inner ranker call (map() returns a new array
  // and would drop the non-enumerable property).
