...
} = require('./tennis-schedule-data/weekly-schedule-2026');
...
async function getTennisContext(opts = {}) {
  ...
  if (start && looksLikeMatchup(rawTournament)) {
    const resolved = resolveTournamentFromMatchup(rawTournament, start);
    if (resolved) {
