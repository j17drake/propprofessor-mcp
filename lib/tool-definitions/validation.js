...
function buildValidationTools() {
  return [
    ...
    },
    {
      ...
      description:
        'Get full details (line history, consensus, movement debug) for specific plays by game ID. Use after a screen call when compact=true hid the data you need. Accepts a singular `book` alias (coerced to books:[book]) or the canonical `books` array.',
      inputSchema: {
        type: 'object',
    ...
    },
    {
      name: 'validate_play',
      description:
        'Run all validation checks on a play in one call: re-fetch screen data, run player_context, check execution quality, return BET/CONSIDER/PASS verdict. PITFALL: always pass playId from a prior quick_screen row — bare selection strings can fail with lookup_failed. Use the league-specific market name (e.g. "Total Points" for WNBA, not "Total").' +
        HONEST_SCOPE_CAVEAT,
      inputSchema: {
        type: 'object',
        usage_example: 'Call with league="NBA", gameId="id-from-screen", selection="Celtics", book="Fliff". For exact matching, pass playId from the screen row instead of (or with) selection — this avoids "no row matched" lookup_failed errors on totals/spread markets.',
        properties: {
          ...
          market: { type: 'string', description: 'Market type, default Moneyline. Required for correct row matching when playId is NOT supplied — use the league-specific market name (e.g. "Total Points", "Point Spread", "Draw No Bet", "Total Games").' },
          books: {
            type: 'array',
            items: { type: 'string' },
            description:
    ...
    },
    {
      ...
      description:
        "Line shopping: show every book's odds sorted best to worst with spread from best price. Auto-resolves league-specific market names. PITFALL: market names vary by league and book. Use get_market_registry first to discover the correct market name for your league+book combo.",
      inputSchema: {
        type: 'object',
