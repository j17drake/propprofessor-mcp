...
function buildValidationTools() {
  return [
    ...
    },
    {
      ...
      description:
        'DRILL DOWN — get full details (line history, consensus, movement debug) for specific plays. Use after a screen call when you need the raw data behind a ranked play.',
      inputSchema: {
        type: 'object',
    ...
    },
    {
      name: 'validate_play',
      description:
        'PRE-BET CHECK — run ALL validation checks on a play in one call: re-fetch screen data, player context, execution quality. Returns BET/CONSIDER/PASS. PITFALL: always pass playId from a prior quick_screen row — bare selection strings can fail with lookup_failed.' +
        HONEST_SCOPE_CAVEAT,
      inputSchema: {
        type: 'object',
    ...
    },
    {
      ...
      description:
        'LINE SHOP — after you identify a play, check which book has the best price. Shows every book sorted best-to-worst. PITFALL: market names vary by league and book. Use get_market_registry first to discover the correct market name.',
      inputSchema: {
        type: 'object',
