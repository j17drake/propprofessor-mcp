  ...
  return [
    {
      name: 'scan',
      description:
        'SCAN — simplified one-call sharp-play scanner. Preferred entry point for AI agents. Wraps quick_screen with smart defaults (NoVigApp, bets-only, sorted by edge). Pass `sport` instead of `league` for ergonomics (e.g. sport="tennis").',
      inputSchema: {
        type: 'object',
        properties: {
          sport: { type: 'string', description: 'Sport to scan: tennis, nba, mlb, nfl, ufc, soccer, etc. Maps to league automatically.' },
          league: { type: 'string', description: 'League name directly (e.g. "Tennis"). Overrides sport if both provided.' },
          book: { type: 'string', description: 'Execution book. Default: NoVigApp.' },
          market: { type: 'string', description: 'Market to scan. Default: Moneyline.' },
          ...
          verbosity: { type: 'string', enum: ['minimal', 'bets', 'standard', 'full'], description: 'Output detail. Default: bets.' },
    ...
    },
    {
      name: 'quick_screen',
      description:
        'SCAN — one-call sharp-play scanner across leagues × markets. Returns ranked results with tiers, edge, movement, and validation.',
      inputSchema: {
        type: 'object',
        ...
        properties: {
          ...
          limit: { type: 'number', description: 'Total max candidates across all leagues/markets, default 100. Use maxPerMarket for per-market caps.' },
          maxPerMarket: { type: 'integer', description: 'Optional per-league×market cap applied before the total limit. E.g. maxPerMarket=2 returns at most 2 plays per league/market pair.' },
    ...
    },
    {
      name: 'smart_bet',
      description:
        'ONE-CALL — evaluate a selection on a book. Returns play details, validation verdict, best price, and staking. For bulk scanning, use quick_screen.',
      inputSchema: {
        type: 'object',
    ...
    },
    {
      name: 'staking_plan',
      description:
        'Given a bankroll, return stake allocations across recommended bets using fractional Kelly staking.',
      inputSchema: {
        type: 'object',
    ...
    },
    {
      name: 'sharp_alerts',
      description:
        'ALERTS — only verified BET-tier plays, deduped so same plays arent re-alerted. Call when you want fresh sharp plays.',
      inputSchema: {
        type: 'object',
