  ...
  return [
    {
      name: 'quick_screen',
      description:
        'SCAN — drill into a book+league after today(). One-call sharp-play scanner: target a book, scan leagues × markets, get ranked results. PITFALL: first call is slow (multi-league fan-out); repeat calls with same args hit cache and return instantly. For "just show me bets", use kaiCall:["BET"] or onlyBets:true.' +
        HONEST_SCOPE_CAVEAT,
      inputSchema: {
        type: 'object',
    ...
    },
    {
      name: 'smart_bet',
      description:
        'ONE-CALL — quick player/team evaluation given a selection and book. Returns play details, validation verdict, best price, and staking. Convenience wrapper that calls quick_screen + validate_play + find_best_price internally. For bulk scanning, use quick_screen directly.' +
        HONEST_SCOPE_CAVEAT,
      inputSchema: {
        type: 'object',
    ...
    },
    {
      name: 'sharp_alerts',
      description:
        'ALERTS — only verified BET-tier plays, deduped so same plays arent re-alerted. No cron/polling — call when you want fresh sharp plays.' +
        HONEST_SCOPE_CAVEAT,
      inputSchema: {
        type: 'object',
