...
function getMarketsForSport(sport, book) {
  ...
  const matchingKey = Object.keys(MARKET_REGISTRY).find(
    (k) => k.toUpperCase() === sportKey.toUpperCase()
  );
  ...
  if (book) {
    ...
    const matchingBookKey = Object.keys(sportEntry).find(
      (k) => k.toUpperCase() === bookKey.toUpperCase()
    );
