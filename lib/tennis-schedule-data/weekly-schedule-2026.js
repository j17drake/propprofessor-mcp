...
// 2026 ATP + WTA + Challenger weekly schedule (June through August).
// Source: https://www.atptour.com/en/scores, https://www.wtatennis.com/tournaments
// Last updated: 2026-06-22
//
// Schema:
//   WEEK_<isoWeekStart> = {
//     start: 'YYYY-MM-DD',  // Monday of the ISO week
//     end:   'YYYY-MM-DD',  // Sunday of the ISO week
//     atp:   [ { name, surface, level, city, slug } ],
//     wta:   [ { name, surface, level, city, slug } ],
//     challenger: [ { name, surface, level, city, slug } ]
//   }
//
// Resolution rules (see resolveTournamentFromMatchup in
// propprofessor-tennis-context.js):
//   1. Pick the week containing the match start timestamp.
//   2. Enumerate active tourneys (ATP, WTA, Challenger).
//   3. For each tourney, check if the matchup's homeTeam or awayTeam is
//      present in the tourney's draw (if a draw list is available).
//   4. The first tourney with a single match is the answer.
//
// The `draws` field is intentionally optional and omitted here because
// per-draw player rosters change daily. The resolver instead uses the
// top-of-week player-to-tourney circuit map (see playerCircuit) plus a
// ranking threshold to pick a tour-level event for top-50 players.
//
// Level constants match the strings returned by guessMatchLevel():
//   'Grand Slam', 'ATP Finals', 'Masters', 'ATP 500', 'ATP 250',
//   'WTA 1000', 'WTA 500', 'WTA 250', 'Challenger', 'ITF Futures'
...
const WEEKLY_SCHEDULE_2026 = {
  ...
  '2026-06-15': {
    ...
    wta: [
      { name: 'Vanda Pharmaceuticals Berlin Tennis Open', surface: 'Grass', level: 'WTA 500', city: 'Berlin',    slug: 'berlin'   },
    ...
    challenger: [
      ...
      { name: 'Nottingham Open Challenger', surface: 'Grass', level: 'Challenger', city: 'Nottingham', slug: 'nottingham-c' }
    ]
  },
  ...
  '2026-06-22': {
    ...
    challenger: [
      ...
      { name: 'Nottingham Challenger',    surface: 'Grass', level: 'Challenger', city: 'Nottingham', slug: 'nottingham-c' }
    ]
  },
  ...
  '2026-07-20': {
    ...
    atp: [
      ...
      { name: 'Winston-Salem Open',   surface: 'Hardcourt', level: 'ATP 250', city: 'Winston-Salem', slug: 'winston-salem' }
    ],
    wta: [
  ...
  '2026-07-27': {
    ...
    wta: [
      ...
      { name: 'Mubadala Silicon Valley Classic', surface: 'Hardcourt', level: 'WTA 250', city: 'San Jose', slug: 'san-jose' }
    ],
    challenger: []
  },
  ...
  '2026-08-03': {
    ...
    wta: [
      { name: 'National Bank Open',     surface: 'Hardcourt', level: 'WTA 1000', city: 'Montreal', slug: 'montreal' }
    ],
    challenger: []
  },
  ...
  '2026-08-10': {
    ...
    atp: [
      { name: 'Cincinnati Open',        surface: 'Hardcourt', level: 'Masters', city: 'Cincinnati', slug: 'cincinnati' }
    ],
    wta: [
      { name: 'Cincinnati Open',        surface: 'Hardcourt', level: 'WTA 1000', city: 'Cincinnati', slug: 'cincinnati' }
    ],
    challenger: []
  }
};
...
const PLAYER_CIRCUIT = {
  ...
  // Both players are in the map under "Zheng" — the resolver picks the
  // first preferred slug in the active week, so an ATP Zheng + a grass week
  // → halle. A WTA Zheng + grass week → bad-homburg.
  'Zhang':     { tour: 'atp', preferredSlugs: ['halle', 'eastbourne', 'wimbledon'] },
  ...
  'Tabur':     { tour: 'atp', preferredSlugs: ['halle', 'eastbourne', 'wimbledon'] },
...
function pickTourneyForMatchup(player1, player2, startIso) {
  ...
  if (c1 && c2) {
    ...
    for (const slug of overlap) {
      const hit = tourneys.find((t) => t.slug === slug);
      if (hit) return hit;
    }
  }
