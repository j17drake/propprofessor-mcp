...
const { gradeMovementQuality, calculateRiskScore, getKaiCall, getConfidenceTier, buildRationale } = require('./propprofessor-risk-score');
const {
  ...
  const presets = {
    ...
    UFC: {
      ...
      marketPriorities: [
        ...
        { match: 'total rounds', weight: 1.7 },
        { match: 'method of victory', weight: 1.6 },
        { match: 'fight goes the distance', weight: 1.6 },
        { match: 'spread', weight: 1.3 }
      ]
    },
    NCAAB: {
...
function rankLeagueScreenRows(
  rows,
  { league = 'NBA', market, limit = 12, includeAll = false, maxAgeMs = null, books = [], recentWindowHours = 6, debug = true } = {}
) {
...
function getResolvedScreenSelection(row) {
  ...
  const orderedSelections = [
    ...
    ...selectionEntries
      .filter(([key]) => !preferredKey || key !== preferredKey)
  ...
  return (Array.isArray(rows) ? rows : [])
    ...
    .map((item) => {
      ...
      return {
        ...
        confidenceTier: getConfidenceTier(item),
...
async function enrichTennisEvCandidates(evCandidates, client, { preferredBook = 'NoVigApp', limit = 12, lookbackHours = 24 } = {}) {
  const sharpBooks = ['Pinnacle', 'Polymarket', 'Circa', 'BetOnline', 'Kalshi'];

  const enriched = [];
  for (const candidate of evCandidates) {
    ...
    const targetBookEntry = sportsbookData.find(sb =>
      String(sb?.book || '').trim() === preferredBook
    );
    const targetBookOdds = targetBookEntry ? Number(targetBookEntry.odds) : null;

    const bestCompEntry = sportsbookData.filter(sb =>
      sharpBooks.includes(String(sb?.book || '').trim())
...
function rankTennisScreenRows(
  rows,
  { limit = 12, preferredBook = 'NoVigApp', includeAll = false, maxAgeMs = null, recentWindowHours = 6, debug = true } = {}
) {
