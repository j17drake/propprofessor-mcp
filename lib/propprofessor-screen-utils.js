...
function extractRowFreshnessMs(row) {
  return extractRowFreshnessInfo(row)?.ms ?? null;
}

function summarizeFreshness(rows, nowMs = Date.now(), options = {}) {
  const sourceRows = Array.isArray(rows) ? rows : [];
  const rowCount = sourceRows.length;
  const freshnessInfos = sourceRows
    .map(extractRowFreshnessInfo)
    .filter(info => info && Number.isFinite(info.ms));
  ...
  const staleCount = Number.isFinite(maxAgeMs) && maxAgeMs >= 0
    ? freshnessMs.filter(age => age > maxAgeMs).length
    : 0;
...
function matchesPreferredBook(bookName, preferredBook) {
  const normalizedBook = String(bookName || '').trim().toLowerCase();
  const normalizedPreferred = String(preferredBook || '').trim().toLowerCase();
...
function rankLeagueScreenRows(rows, { league = 'NBA', market, limit = 12, includeAll = false, maxAgeMs = null, books = [], debug = true } = {}) {
...
function rankScreenRows(rows, { limit = 12, preferredBooks = ['NoVigApp', 'Polymarket', 'Kalshi', 'BetOnline', 'Circa'], includeAll = false, maxAgeMs = null, recentWindowHours = 6, debug = true } = {}) {
  ...
  const sharpBooks = Array.isArray(preferredBooks) ? preferredBooks.map(book => String(book || '').trim()).filter(Boolean) : [];
  return (Array.isArray(rows) ? rows : [])
    ...
    .map(row => {
      ...
      const fallbackCurrentOdds = trail.length >= 2 ? trail[trail.length - 1] : extractNumericTrailValue({ odds: row?.odds || row?.currentOdds || row?.price });
      ...
      const fallbackClvProxyPct = Number.isFinite(fallbackOpeningProb) && Number.isFinite(fallbackCurrentProb)
      ...
      const freshnessAgeMs = Number.isFinite(freshnessMs) ? Math.max(0, Date.now() - freshnessMs) : null;
      const freshnessSource = freshnessInfo?.source || 'response_received';
      const freshnessFallbackUsed = !freshnessInfo;
      const isStale = Number.isFinite(numericMaxAgeMs) && Number.isFinite(freshnessMs) ? freshnessAgeMs > numericMaxAgeMs : false;
      ...
      const movementReason = movementSummary.lineHistoryUsable
        ...
        : (Number.isFinite(clvProxyPct) ? `legacy CLV proxy ${clvProxyPct.toFixed(2)}%` : 'no usable sharp movement');
...
function rankTennisScreenRows(rows, { limit = 12, preferredBook = 'NoVigApp', includeAll = false, maxAgeMs = null, debug = true } = {}) {
  const tennisRows = Array.isArray(rows) ? rows.filter(isTennisRow) : [];
  return rankScreenRows(tennisRows, { limit, preferredBooks: [preferredBook, 'NoVigApp', 'Polymarket', 'Kalshi', 'BetOnline', 'Circa'], includeAll, maxAgeMs, recentWindowHours: 6, debug: debug === undefined ? true : debug }).map(row => ({
    ...row,
