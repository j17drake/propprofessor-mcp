...
function inspectAuthSetup() {
  ...
  const source = getExplicitAuthFile()
    ? 'AUTH_FILE'
    : selectedAuthFile === DEFAULT_USER_AUTH_FILE
      ? 'user'
      : 'repo';

  return {
    ok: selectedEntry.exists && selectedEntry.readable && selectedEntry.parseable && selectedEntry.propProfessorCookieCount > 0,
    source,
    selectedAuthFile,
...
async function fetchWithTimeout(fetchImpl, url, options, { timeoutMs = DEFAULT_REQUEST_TIMEOUT_MS, source = 'request' } = {}) {
