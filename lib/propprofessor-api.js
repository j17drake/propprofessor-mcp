...
function getOddsHistoryStartTimestamp({ lookbackHours = 12, nowMs = Date.now() } = {}) {
  ...
  return Math.max(0, Math.floor((safeNow - (safeHours * 60 * 60 * 1000)) / 1000));
}
...
function createPropProfessorClient({
  ...
  return {
    ...
    queryOddsHistory({ gameId, selectionId, startTimestamp, sportsbooks = [], lookbackHours = 12, nowMs = now() } = {}) {
      if (!gameId) {
