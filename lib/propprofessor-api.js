...
function createPropProfessorClient({
  ...
  return {
    ...
    queryScreenOdds(filters = {}) {
      ...
      const books = normalizedFilters.books !== undefined && normalizedFilters.books.length
    ...
    queryScreenOddsBestComps(filters = {}) {
      const normalizedFilters = filters && typeof filters === 'object' ? filters : {};
      const hasBooks = normalizedFilters.books !== undefined && normalizedFilters.books.length > 0;
      const merged = {
        ...normalizedFilters,
        books: hasBooks
          ? normalizedFilters.books
