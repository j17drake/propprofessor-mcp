...
const BETS_MODE_FIELDS = new Set([
  'game',
  'startCST',
  'selection',
  'odds',
  'market',
  'league',
  // Single authoritative verdict — consolidateVerdict resolves the chain
  'verdict',
  'tier',
  'movement',
  'edge',
...
function formatBetCompact(bet = {}) {
  ...
  return {
    ...
    verdict: consolidateVerdict(bet),
    ...
    books: bet.validatedConsensusBookCount ?? bet.consensusBookCount ?? 0,
...
function formatQuickScreenMinimal(response = {}) {
  ...
  out.plays = allPlays.map((p) => ({
    ...
    edge: p.edge,
    startCST: p.startCST,
    movementDisposition: p.movementDisposition,
