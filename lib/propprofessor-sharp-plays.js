...
function classifySharpPlay(row = {}, options = {}) {
  ...
  // Consensus-validated fallback: enough books agree on edge, movement is
  ...
  // from all books' implied probabilities — but we can't confirm independent
  // sharp movement. Produces Lean (never Bet candidate) for safety.
  const consensusOkNoSharpMovement =
    consensusOk &&
    hasConsensusEdge &&
    movementLabelSupported &&
    !movementIsSharpSourced &&
  ...
  if (isPropMarket && propClassifyOk) {
    ...
    verdict = 'Lean';
  } else if (!strict && consensusOkNoSharpMovement) {
    verdict = 'Lean';
  }
