...
function classifySharpPlay(row = {}, options = {}) {
  ...
  // no longer produce Bet candidate. All Bet candidates must have actual
  ...
  // These flags are kept for pass-reason diagnostics only.
  const consensusEdgeOnlyOk = false;
  const consensusOnlyOk = false;
  const clvOnlyOk = false;
  ...
  if (!consensusOk && !isPropMarket && !alternativeBookCountsOk) passReasons.push(`consensus_book_count_below_${minConsensusBookCount}`);
