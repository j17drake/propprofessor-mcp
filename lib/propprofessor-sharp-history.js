...
function summarizeSharpMovement({ lineHistory, preferredBook, sharpBooks, options = {} }) {
  ...
  return {
    ...
    minClvPct: windows.fullWindow?.minClvPct ?? null,
    peakAdverseClvPct: windows.fullWindow?.minClvPct ?? null,
    clvProxyPct: windows.fullWindow?.clvPct ?? null,
    recentClvPct: windows.recentWindow?.clvPct ?? null,
    recentWindowHours:
      windows.recentWindow?.windowHours ??
      (Number.isFinite(Number(options.recentWindowHours)) ? Number(options.recentWindowHours) : 6),
