...
function matchPlay(row, { game, market, selection }) {
  ...
  if (targetSelection && !rowSelection.includes(targetSelection) && !targetSelection.includes(rowSelection)) return false;
...
function collectBookOddsFromRow(row, selectionFilter) {
  ...
  for (const selection of selections) {
    ...
    const includeSide1 = !filterKey || normalizeSelectionKey(selection1Name).includes(filterKey) || filterKey.includes(normalizeSelectionKey(selection1Name));
    const includeSide2 = !filterKey || normalizeSelectionKey(selection2Name).includes(filterKey) || filterKey.includes(normalizeSelectionKey(selection2Name));
