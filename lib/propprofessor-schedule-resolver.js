...
const { getTodayTennisSchedule, getGameIdFromSchedule } = require('./propprofessor-schedule');
...
async function resolveGamesWithScheduleFirst(params) {
  ...
  try {
    ...
    if (!scheduleGames || scheduleGames.length === 0) {
      console.log('[schedule-resolver] No games found in schedule');
      return [];
    }
    ...
    if (todayGameIds.length === 0) {
      console.log('[schedule-resolver] No games scheduled for today');
      return [];
    }
    ...
    console.log('[schedule-resolver] Falling back to traditional screen query');
