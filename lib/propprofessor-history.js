...
async function resolveHistoryForEntity({ client, target, rows, queryHistoryFn, lookbackHours = getOddsHistoryLookbackHours(), nowMs = Date.now(), historySportsbooks = [], preferredBook = null, sharpBooks = [] }) {
  ...
  return {
    ...
    historyMatchedBy: matchStrength.selectionIdMatch ? 'selectionId' : matchStrength.gameIdMatch ? 'gameId' : 'fallback',
