...
function normalizeDashboardFilters(input = {}) {
  ...
  return {
    ...
    compactMode: getBooleanParam(params, 'compactMode', false),
...
function buildQueryString(filters = {}, overrides = {}) {
  ...
  if (merged.compactMode) params.set('compactMode', '1');
...
function renderFilters(filters) {
  ...
  const compactModeClass = filters.compactMode ? 'compact-mode' : '';
  ...
  return `
    <form class="filters command-bar ${escapeHtml(compactModeClass)}" method="GET" action="/">
      ...
      <div class="command-primary">
        <div class="compact-toggle">
          <label class="mode-toggle"><input type="hidden" name="compactMode" value="0" /><input name="compactMode" type="checkbox" value="1" ${checked(filters.compactMode)} /><span>Compact mode</span></label>
        </div>
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    /* Command bar - responsive and compact */
    .filters { display:block; padding:16px; margin-bottom:16px; }
    .command-primary { display:grid; grid-template-columns:minmax(240px,1.4fr) repeat(3,minmax(120px,.65fr)) minmax(110px,.5fr) auto; gap:12px; align-items:end; }
    .command-label { color:var(--muted); font-size:11px; text-transform:uppercase; letter-spacing:.09em; font-weight:800; }
    .book-toggle-row, .quick-presets, .empty-actions { display:flex; flex-wrap:wrap; gap:8px; margin-top:7px; }
    .book-toggle { display:flex; flex-direction:row; align-items:center; gap:8px; padding:8px 11px; border:1px solid var(--line); border-radius:999px; background:#071020; color:#d6e2f5; text-transform:none; letter-spacing:0; font-size:12px; cursor:pointer; }
    ...
    .compact-toggle { order:0; }
    .mode-toggle { flex-direction:row; align-items:center; gap:8px; justify-content:center; border:1px solid var(--line); border-radius:12px; background:#071020; padding:9px 11px; color:#d6e2f5; text-transform:none; letter-spacing:0; font-size:12px; cursor:pointer; }
    .mode-toggle input { width:auto; accent-color:var(--blue); }
    .advanced-filters { margin-top:10px; border-top:1px solid var(--line); padding-top:10px; }
    .advanced-filters summary { color:var(--blue); cursor:pointer; font-weight:700; font-size:12px; }
    .advanced-grid { display:grid; grid-template-columns:repeat(4, minmax(110px, 1fr)); gap:10px; padding-top:10px; }
    .quick-presets a, .empty-actions a { color:#d6e2f5; border:1px solid var(--line); background:#071020; border-radius:999px; padding:6px 9px; font-size:11px; }
    .hero-meta { display:flex; flex-wrap:wrap; gap:7px; margin-top:14px; }
    .hero-meta span { border:1px solid var(--line); background:#071020; color:#d6e2f5; border-radius:999px; padding:5px 8px; font-size:11px; }
    ...
    .price-strip .best-price { border-color:rgba(51,214,159,.46); color:var(--good); background:rgba(51,214,159,.10); font-weight:900; font-size:11px; }
    .compact-mode .play-card .market-explanation, .compact-mode .play-card .timeline { display:none; }
    .compact-mode .price-strip span { font-size:11px; padding:5px 8px; }
    .compact-mode .chips span, .compact-mode .triage-meta span { font-size:11px; padding:4px 7px; }
    ...
    .actions { grid-column:2 / -1; margin-top:8px; }
    .actions button { padding:7px 9px; border-radius:11px; font-size:11px; }
    ...
    @media (max-width: 1200px) {
      .command-primary { grid-template-columns:minmax(200px,1.5fr) repeat(2,minmax(110px,.7fr)) auto; }
      .advanced-grid { grid-template-columns:repeat(4, 1fr); }
    }
    @media (max-width: 900px) {
      .command-primary { grid-template-columns:1fr 1fr; }
      .hero { grid-template-columns:1fr; }
      .hero-copy, .hero-stats { grid-column:1 / -1; }
      .content { grid-template-columns:1fr; }
      .play-card { grid-template-columns:40px 1fr; }
      .metric, .actions, .research-box, .reasons { grid-column:1 / -1; text-align:left; }
    }
    @media (max-width: 600px) {
      .command-primary { grid-template-columns:1fr; }
      .advanced-grid { grid-template-columns:1fr; }
      .book-toggle-row, .quick-presets, .empty-actions { flex-direction:column; }
      .play-card { grid-template-columns:36px 1fr; }
      .rank { font-size:18px; }
