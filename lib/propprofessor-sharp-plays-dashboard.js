...
function normalizeDashboardFilters(input = {}) {
  ...
  return {
    book: normalizeBookName(get('book') || DEFAULT_BOOK),
...
function formatAge(ms) {
  ...
  if (!Number.isFinite(number)) return 'fresh';
  if (number < 60_000) return `${Math.max(0, Math.round(number / 1000))}s`;
...
function buildQueryString(filters = {}, overrides = {}) {
  ...
  params.set('book', merged.book || DEFAULT_BOOK);
...
function summarizeBookPrices(play = {}) {
...
function renderFilters(filters) {
  ...
  return `
    <form class="filters" method="GET" action="/">
      <label>Target book
        <select name="book">${renderOptions(TARGET_BOOK_OPTIONS, filters.book)}</select>
      </label>
      <label>League
      ...
      <label class="check"><input type="hidden" name="broad" value="0" /><input name="broad" type="checkbox" value="1" ${checked(!filters.strict)} /> broad</label>
...
function renderPlayCard(play, index, context = {}) {
  ...
  const actionLabel = verdict.label === 'Research first' && priceStatus.status === 'Playable' && verdict.directTargetBookVerified ? 'Bet' : verdict.label;
  const actions = [
    ...
    <article id="play-${escapeHtml(researchId)}" class="play-card ${labelClass(actionLabel)}" data-play-id="${escapeHtml(researchId)}">
      ...
      <div class="play-main">
        ...
        <div class="chips">
          ...
          <span>${escapeHtml(play.book || support.targetBook || '')} ${escapeHtml(formatOdds(play.odds))}</span>
...
function renderEmpty(snapshot) {
  ...
  return `
    <section class="empty">
      <h2>No clean sharp-supported plays right now</h2>
      <p>Scanned ${escapeHtml(scanned)} ranked rows. Try broad mode or show passes to see what failed.</p>
    </section>
  `;
}
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .chips, .timeline, .triage-meta, .actions { display:flex; flex-wrap:wrap; gap:7px; margin-top:10px; }
    ...
    @media (max-width: 1100px) { .hero, .content { grid-template-columns:1fr; } .filters { grid-template-columns:1fr 1fr; } .play-card { grid-template-columns:42px 1fr; } .metric, .actions, .research-box, .reasons { grid-column:1 / -1; text-align:left; } }
  </style>
</head>
<body>
  <main class="shell">
    <header class="hero">
      <section class="hero-copy">
        ...
        <p class="lede">Premium hybrid shell with sports-trading density. Sort actionable plays first, queue research when needed, and persist placed / hidden / pass decisions so the same rows stop wasting attention.</p>
      </section>
      <section class="hero-stats">
        <div class="stat"><strong>${escapeHtml(getHeroStats(filtered, researchEntries, decisionEntries).actionableNow)}</strong><span>Actionable now</span></div>
        <div class="stat"><strong>${escapeHtml(getHeroStats(filtered, researchEntries, decisionEntries).researchQueued)}</strong><span>Research queued/running</span></div>
        <div class="stat"><strong>${escapeHtml(getHeroStats(filtered, researchEntries, decisionEntries).researchDoneNeedsDecision)}</strong><span>Research done / needs decision</span></div>
        <div class="stat"><strong>${escapeHtml(getHeroStats(filtered, researchEntries, decisionEntries).hiddenOrPlaced)}</strong><span>Hidden / placed</span></div>
      </section>
    </header>
    ...
    <div class="content">
      <section>
        ...
        ${filtered.length ? `<section class="board">${filtered.map((play, index) => renderPlayCard(play, index, { filters, researchEntries, decisionEntries })).join('')}</section>` : renderEmpty(snapshot)}
      </section>
