...
async function fetchViaXurl({ player, sport, maxResults = 20 }) {
  ...
  try {
    const { stdout } = await pExecFile('xurl', ['search', query, '-n', String(maxResults)], { timeout: 15_000 });
    let parsed;
    try {
      parsed = JSON.parse(stdout);
    } catch {
      return { source: 'xurl-failed', error: 'xurl returned non-JSON output', tweets: [], news: [] };
    }
    // 401/403 from xurl (no auth) — surface a clean error instead of treating as 0 results
    ...
    return { source: 'xurl-failed', error: err.message || String(err), tweets: [], news: [] };
  }
}
