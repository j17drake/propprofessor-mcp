...
function rankLeagueScreenRows(rows, { league = 'NBA', limit = 12, includeAll = false, maxAgeMs = null, books = [] } = {}) {
  const marketHint = Array.isArray(rows)
    ? rows.map(row => row?.market || row?.selection || '').find(Boolean)
    : '';
  const preset = getLeagueRankingPreset(league, marketHint);
...
function rankScreenRows(rows, { limit = 12, preferredBooks = ['NoVigApp', 'Polymarket', 'Kalshi', 'BetOnline', 'Circa'], includeAll = false, maxAgeMs = null } = {}) {
  ...
  return (Array.isArray(rows) ? rows : [])
    ...
    .map(row => {
      const rowMarket = row?.market || getTennisMarketName(row) || row?.selection || '';
      const leaguePreset = getLeagueRankingPreset(row?.league || row?.sport || row?.gameType || '', rowMarket);
      const rowMarketName = String(rowMarket).toLowerCase();
