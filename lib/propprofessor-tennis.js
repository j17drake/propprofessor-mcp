 ...
 *   - propprofessor-tennis-times.js — was a stub for ESPN-backed time correction
 *
 * No behavior change vs. v1.7.0. The "ESPN-backed match time correction" was
 * never wired into the validate_play or quick_screen pipeline — the upstream
 * `start` field flows through unchanged. The tennis-context cache fix in
 * v2.3.2 (cache key includes start + imminent-match freshness check, see
 * lib/propprofessor-game-context.js) handles staleness on the read side.
 *
 * TODO(2.4.0): wire a real ESPN scoreboard call to reconcile `start` for
 * imminent matches. Until then, callers must defend against stale start
 * times via the find_best_price book-liquidity check (see
 * propprofessor-bet-card rule H in ~/.hermes/skills/propprofessor-bet-card/SKILL.md).
 */
