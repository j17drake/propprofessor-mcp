...
async function runResearchOnTopRows({ rows, limit = 10, playerContextFn, gameContextFn, maxAgeMinutes = 60, concurrency = 3 } = {}) {
  ...
  const results = await mapWithConcurrency(
    ...
    async (row) => {
      ...
      try {
        ...
        if (!ctx) {
          return { player, game, league, market, start, riskFlag: 'unknown', contextType: isPlayer ? 'player' : 'game', error: 'no_context' };
        }
