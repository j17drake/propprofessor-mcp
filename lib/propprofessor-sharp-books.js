...
// The /api/retrieve-data-new endpoint only returns multi-book data for
// secondary sports (Tennis, Soccer, etc.) when the complete list is passed.
const ALL_SCREEN_BOOKS = [
  '4cx', 'BallyBet', 'Bet105', 'BetMGM', 'BetOnline', 'BetParx', 'BetRivers',
  'BookMaker', 'Bovada', 'Caesars', 'Circa', 'DraftKings', 'Fanatics',
  'FanaticsMarkets', 'FanDuel', 'Fliff', 'Kalshi', 'NoVigApp', 'OnyxOdds',
  'Pinnacle', 'Polymarket', 'Prop Builder', 'Prophet', 'Rebet', 'theScore',
  'PrizePicks', 'Betr', 'Dabble', 'DraftKings6', 'OwnersBox', 'Sleeper',
  'ParlayPlay', 'HotStreak', 'BoomFantasy', 'Betr (Alt)', 'Dabble (Alt)',
  'DraftKings6 (Alt)', 'Rebet (Alt)', 'Underdog (Alt)'
];
...
function uniqueBooks(books) {
  return Array.from(
    new Set((Array.isArray(books) ? books : []).map((book) => String(book || '').trim()).filter(Boolean))
  );
}
