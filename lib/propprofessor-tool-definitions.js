...
const LITE_MODE_TOOLS = new Set([
  ...
  'get_market_registry', // discover markets for a sport/book
  ...
  'smart_bet' // one-call bet evaluation (quick_screen + validate_play + line shop + staking)
]);
 ...
 * @param {('full'|'lite')} [options.mode='full'] - 'full' returns all 27 tools;
 *   'lite' returns only the 11 essentials (discovery, router, screen, drill-down,
...
function buildToolDefinitions({ mode = 'full' } = {}) {
  ...
  const definitions = [
      ...
      inputSchema: {
        ...
        properties: {
          ...
          validateTop: {
            ...
            default: 0,
            description:
              'When > 0, runs validate_play on the top N candidates per league/market before returning, merging verdictSummary and gameContext data into each row. Eliminates the need for a separate validate_play call. The agent gets validated tiers and consensus counts in one round-trip.'
          }
        },
        additionalProperties: false
      }
    ...
    },
    {
      name: 'live_monitor',
      description:
        'Monitor a specific play for line movement. Returns current odds across all books and compares them to a target odds threshold. Use to track whether a line improves to your target price. Pair with a cron job for periodic checks.' +
        HONEST_SCOPE_CAVEAT,
      inputSchema: {
        type: 'object',
        properties: {
          selection: { type: 'string', description: 'Player or team name. Required.' },
          league: { type: 'string', description: 'League (e.g. "NBA"). Required.' },
          market: { type: 'string', description: 'Market type, default "Moneyline".' },
          game: { type: 'string', description: 'Game matchup string (e.g. "Lakers @ Warriors"). Narrower matching.' },
          targetOdds: { type: 'number', description: 'Target American odds threshold (e.g. +150). Alert when odds reach or exceed this value.' }
        },
        required: ['selection', 'league'],
        additionalProperties: false
      }
    },
    {
