...
function buildToolDefinitions() {
  return [
    ...
    },
    {
      ...
      description:
        'Get recent news, tweets, and injury context for a player. Returns up to 30 recent tweets mentioning the player from X, with author, timestamp, and engagement metrics. Use before placing a bet to check for breaking news or injury reports. Returns empty `tweets` array and `source: "empty"` if X returns no results (Phase 2 will add a Google News RSS fallback for reliability).',
      inputSchema: {
        type: 'object',
        properties: {
