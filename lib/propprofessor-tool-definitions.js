  ...
  return [
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          is_live: { type: 'boolean', description: 'Whether to query live +EV rows' },
    ...
    },
    {
      ...
      description:
        ...
        HONEST_SCOPE_CAVEAT +
        ' Supercedes novig_screen for all new code — quick_screen works with any book, not just NoVigApp.',
      inputSchema: {
        type: 'object',
        properties: {
          books: {
    ...
    },
    {
      name: 'screen',
      description:
        "DEPRECATED: prefer screen_ranked({ league, market }) for the same behavior. This tool is functionally equivalent to screen_ranked with a single market filter; kept for backward compatibility. New code should call screen_ranked. Generic market names like Total or Spread are auto-resolved per league — e.g. NHL Total becomes Total Goals, MLB Spread becomes Run Line. Each row includes `consensusStrength` (strong/moderate/weak/none). When aliases are resolved, `responseMeta.markets_alias_used` is set. NOTE: When you pass a single non-sharp book (e.g. NoVigApp), the tool auto-augments the query with the league's sharp book set (Pinnacle, BetOnline, Circa, etc.) so consensus and movement data populate." +
        HONEST_SCOPE_CAVEAT,
      inputSchema: {
        type: 'object',
        properties: {
      ...
      inputSchema: {
        ...
        properties: {
          targetBooks: {
            ...
            items: { type: 'string' },
            description:
              'Target execution books to scan together, such as ["Fliff", "NoVigApp"]. Preferred over singular book.'
          },
          books: {
            type: 'array',
            items: { type: 'string' },
            description: 'Alias for targetBooks. Use either books or targetBooks.'
          },
          targetBooksCsv: { type: 'string', description: 'Dashboard URL convenience only — prefer targetBooks.' },
          book: {
            type: 'string',
            description: 'Legacy single target execution book such as Fliff, NoVigApp, or Rebet. Prefer targetBooks.'
          },
          targetBook: { type: 'string', description: 'Legacy alias for book. Prefer targetBooks.' },
          leagues: {
            type: 'array',
            items: { type: 'string' },
            description:
    ...
    },
    {
      ...
      description:
        ...
        HONEST_SCOPE_CAVEAT +
        ' EQUIVALENT: this is essentially sharp_plays({ targetBooks: ["NoVigApp"], includeResearch: true }) with relaxed price requirements. Prefer sharp_plays for new code.',
      inputSchema: {
        type: 'object',
        properties: {
          leagues: {
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          is_live: { type: 'boolean', description: 'Whether to query live plays.' },
          periodTypes: {
            type: 'array',
            items: { type: 'string' },
