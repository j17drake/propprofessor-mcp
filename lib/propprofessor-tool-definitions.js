...
function buildToolDefinitions() {
  ...
  return [
    ...
    {
      name: 'health_status',
      description: 'Check auth freshness and endpoint connectivity. Reports token persistence state: `persistedToDisk` (boolean), `refreshCount` (number of refreshes since startup), and `lastRefreshed` (ISO timestamp).',
      inputSchema: { type: 'object', properties: {}, additionalProperties: false }
    },
    {
    ...
    },
    {
      ...
      description: 'Line shopping: show every book\'s odds sorted best to worst with spread from best price. Generic market names like Total or Spread are auto-resolved per league — e.g. NHL Total becomes Total Goals, MLB Spread becomes Run Line.',
      inputSchema: {
        type: 'object',
        properties: {
