...
function buildToolDefinitions() {
  ...
  return [
    ...
    },
    {
      name: 'screen_ranked',
      description:
        'Query /screen and return hydrated ranked rows with consensus, movement, and freshness metadata for any market. This is the primary tool for getting tiered, ranked plays. Set compact=true to strip verbose payloads (lineHistory, scoreBreakdown, full odds maps) and return only essential fields per match — reduces response size by ~90%. NOTE: When you pass a single non-sharp book (e.g. NoVigApp), the tool auto-augments the query with the league\'s sharp book set (Pinnacle, BetOnline, Circa, etc.) so consensus and movement data populate. Your requested book is preserved as the focus/execution book for display.',
      inputSchema: {
        type: 'object',
        properties: {
          market: { type: 'string', description: 'Odds screen market, for example Moneyline or Player Points' },
          ...
          compact: {
            type: 'boolean',
            description: 'When true, strip verbose payloads (lineHistory, scoreBreakdown, full odds maps) and return only essential fields per match. Reduces response size by ~90%. Default false. Does NOT affect history hydration — movement data is always fetched.'
          },
          fields: {
            type: 'array',
            items: { type: 'string' },
            description: 'Optional list of field names to return per row. Overrides compact when both are set. Example: ["game","selection","odds","edge","tier","kai"]'
          },
          skipHistory: {
            type: 'boolean',
            description: 'When true, skip odds history hydration entirely. Use this to speed up screen calls when you only need current odds/edges and do not need movement data. Default false.'
          },
          include: {
            type: 'array',
            items: { type: 'string' },
            description: 'Optional list of top-level response sections to include. Values: "freshness", "warnings", "resultMeta", "league". Example: ["resultMeta"] to get only ok+result+resultMeta. Default: all sections.'
          },
          verbosity: VERBOSITY_PARAM
        },
        required: ['league'],
    ...
    },
    {
      name: 'screen',
      description:
        'Query /screen for any supported league and return league-specific ranked rows. Replaces all per-league screen tools (query_nba_screen, query_mlb_screen, etc.). Use this as the default league screen tool. NOTE: When you pass a single non-sharp book (e.g. NoVigApp), the tool auto-augments the query with the league\'s sharp book set (Pinnacle, BetOnline, Circa, etc.) so consensus and movement data populate. Your requested book is preserved as the focus/execution book for display.',
      inputSchema: {
        type: 'object',
        properties: {
          league: {
    ...
    },
    {
      name: 'sharp_plays',
      description:
        'Add-on scanner for the best target-book plays with supportive sharp movement. Queries /screen across leagues/markets, hydrates odds history, and only treats non-target sharp-book movement as support. Defaults to scanning Moneyline, Spread, and Total markets. Pass `markets` to override.',
      inputSchema: {
        type: 'object',
        properties: {
          targetBooks: {
          ...
          requireIndependentSharpMovement: { type: 'boolean', description: 'Require non-target sharp support when true' },
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          minConsensusWindows: { type: 'number', description: 'Minimum consensus windows required to include a play, default 0' },
          sharpBooks: {
            type: 'array',
            items: { type: 'string' },
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          market: { type: 'string', description: 'Market filter applied across all leagues, default Moneyline. Deprecated: use `markets` for multi-market scans.' },
          markets: {
            type: 'array',
            items: { type: 'string' },
            description: 'Market types to scan. Default: ["Moneyline", "Spread", "Total"]. Overrides `market` if provided.'
          },
          compact: {
            type: 'boolean',
            description: 'When true, strip verbose payloads (lineHistory, scoreBreakdown, full odds maps) and return only essential fields per match. Reduces response size by ~90%. Default false. Does NOT affect history hydration — movement data is always fetched.'
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          market: { type: 'string', description: 'Market type, default "Moneyline". Deprecated: use `markets` for multi-market scans.' },
          markets: {
            type: 'array',
            items: { type: 'string' },
            description: 'Market types to scan. Default: ["Moneyline", "Spread", "Total"]. Overrides `market` if provided.'
    ...
    },
    {
      name: 'staking_plan',
      description:
        'Given a bankroll and optional play filter, return stake allocations across recommended bets. Uses fractional Kelly staking: TIER 1 = 2%, TIER 2 = 1% of bankroll. Includes total exposure, per-play stake dollars, and correlation warnings. Defaults to scanning Moneyline, Spread, and Total markets via `recommended_bets`.',
      inputSchema: {
        type: 'object',
        properties: {
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          useXurl: { type: 'boolean', description: 'Escalate to paid xurl CLI for real-time results. Bypasses cache. Use sparingly.' }
        },
        required: ['player'],
        additionalProperties: false
      }
    },
    {
      ...
      description:
        'Get full details (including line history, consensus, movement debug) for specific plays by game ID. Use after a compact/fields screen query to drill into selected plays. Returns full rows with all metadata.',
      inputSchema: {
        type: 'object',
        properties: {
          league: {
    ...
    {
      name: 'health_status',
      description: 'Check auth freshness and endpoint connectivity.',
      inputSchema: { type: 'object', properties: {}, additionalProperties: false }
    },
    {
    ...
    },
    {
      ...
      description: 'Line shopping: show every book\'s odds sorted best to worst with spread from best price.',
      inputSchema: {
        type: 'object',
        properties: {
