...
function buildToolDefinitions() {
  return [
    ...
    {
      name: 'query_clv_history',
      description:
        'Read your bet log CSV (default: ~/Documents/bet-log.csv) and compute your actual CLV track record over a lookback window. Returns average CLV%, CLV stddev, win rate, profit, ROI, plus per-group breakdowns (by week, sport, tier, or book). The single best predictor of long-term profitability — if your CLV is positive over 100+ bets, you have an edge. Required CSV columns: date, league, market, selection, book, odds_taken, closing_odds, outcome (win/loss/push), stake, tier.',
      inputSchema: {
        type: 'object',
        properties: {
          days: { type: 'number', description: 'Lookback window in days, default 30' },
          groupBy: { type: 'string', description: 'One of: week, sport, tier, book. Default week.' },
          path: { type: 'string', description: 'Override bet log CSV path. Default: ~/Documents/bet-log.csv. Also reads BET_LOG_PATH env var.' }
        },
        additionalProperties: false
      }
    },
    {
      name: 'query_player_context',
      description:
        'Get recent news, tweets, and a computed risk flag for a player. Returns up to 30 recent tweets mentioning the player from X plus a Google News RSS layer (with ESPN as a tertiary fallback). Each item is scored 0-100 for source authority against a curated beat-reporter watchlist. The `riskFlag` field is `clean` (no injury keywords), `monitor` (injury keyword in last hour, lower-authority), or `high` (high-authority source like a beat reporter + injury keyword in last 2h). USE THIS BEFORE PLACING A BET: if `riskFlag === "high"`, downgrade or skip the play. The watchlist lives at ~/.hermes/skills/pp-sports/references/beat-reporter-watchlists.md and is re-read on every call — no redeploy needed to add reporters.',
      inputSchema: {
        type: 'object',
        properties: {
          player: { type: 'string', description: 'Player full name (e.g., "Frances Tiafoe")' },
          sport: { type: 'string', description: 'Sport name (e.g., "Tennis", "NBA", "MLB"). Tennis queries the player name only; other sports append the sport to the query.' },
          gameTime: { type: 'string', description: 'Optional ISO timestamp of the game (used in later phases for weather + recency weighting)' },
          maxAgeMinutes: { type: 'number', description: 'How far back to search, default 60. Currently informational only — Phase 1 always returns the latest tweets from X.' },
          useXurl: { type: 'boolean', description: 'Escalate to the paid xurl CLI for real-time results. Bypasses cache. Use sparingly — only for critical injury checks (sub-30-min games) or when the free path returned empty. Requires `xurl auth oauth2` setup; will return source: "xurl-failed" with a setup hint if not configured.' }
        },
        required: ['player'],
        additionalProperties: false
      }
