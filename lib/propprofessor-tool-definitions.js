 ...
 * can opt back into the full 28-tool surface via PROPPROFESSOR_MCP_MODE=full.
 */
 ...
 * @param {('full'|'lite')} [options.mode='full'] - 'full' returns all 28 tools;
 *   'lite' returns only the 12 essentials (discovery, router, screen, drill-down,
...
function buildToolDefinitions({ mode = 'full' } = {}) {
  ...
  const definitions = [
      ...
      inputSchema: {
        ...
        properties: {
          ...
          validateTop: {
            ...
            default: 3,
            description:
              'When > 0, runs validate_play on the top N candidates per league/market before returning, merging verdictSummary and gameContext data into each row. Defaults to 3 — every quick_screen call returns validated tiers and consensus counts. Set to 0 to skip validation for faster results.'
          }
        },
        additionalProperties: false
      }
    ...
    },
    {
      ...
      inputSchema: {
        type: 'object',
        usage_example: 'Call with game="Lakers vs Celtics", league="NBA", market="Moneyline", selection="Lakers", odds=-150.',
        properties: {
