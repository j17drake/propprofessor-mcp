...
function buildToolDefinitions() {
  return [
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          compact: {
            type: 'boolean',
            description: 'When true, strip verbose payloads (lineHistory, scoreBreakdown, full odds maps) and return only essential fields per match. Reduces response size by ~90%. Default false.'
          },
          fields: {
            type: 'array',
            items: { type: 'string' },
    ...
    {
      name: 'query_bet_stats',
      description: 'Aggregate recorded bet outcomes from the local event log. Returns counts and profit by requested grouping dimensions.',
      inputSchema: {
        type: 'object',
        properties: {
          since: { type: 'string', description: 'ISO timestamp to filter outcomes from.' },
          groupBy: { type: 'string', description: 'Comma-separated grouping fields, e.g. league,market,tier' }
        },
        additionalProperties: false
      }
    },
    ...
    },
    {
      ...
      description:
        'Return only the highest-confidence plays across requested leagues, ranked by confidence. Each row includes movementGrade, riskScore (1-10), kaiCall (BET/CONSIDER/PASS), confidenceTier (TIER 1-4), and a human-readable rationale string. Use this as your main what-to-bet-today tool.',
      inputSchema: {
        type: 'object',
        properties: {
          books: {
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          path: { type: 'string', description: 'Override bet log CSV path. Default: ~/Documents/bet-log.csv or BET_LOG_PATH env var.' }
        },
        additionalProperties: false
      }
    },
    ...
    {
      name: 'record_outcome',
      description: 'Record a bet outcome tied to a recommendation or personal bet id. Use this to close the loop and improve future suggestions.',
      inputSchema: {
        type: 'object',
        properties: {
          betId: { type: 'string', description: 'Stable bet identifier from a recommendation or your own tracking id.' },
          outcome: { type: 'string', description: 'One of: win, loss, push.' },
          profit: { type: 'number', description: 'Net profit/loss for this bet. Use negative numbers for losses.' },
          source: { type: 'string', description: 'Source of the bet, e.g. screen, ev_candidates, manual.' },
          league: { type: 'string', description: 'League for this outcome, e.g. NBA, Tennis, UFC.' },
          market: { type: 'string', description: 'Market for this outcome, e.g. Moneyline, Player Points.' },
          notes: { type: 'string', description: 'Optional freeform notes.' }
        },
        required: ['betId', 'outcome', 'profit'],
        additionalProperties: false
      }
    },
    {
      name: 'record_feedback',
      description: 'Record qualitative feedback for a recommendation. Use this to explain why a play was skipped or rejected.',
      inputSchema: {
        type: 'object',
        properties: {
          betId: { type: 'string', description: 'Stable bet identifier from a recommendation or personal bet id.' },
          skipReason: { type: 'string', description: 'Why this play was skipped or rejected.' },
          source: { type: 'string', description: 'Source of the bet, e.g. screen, ev_candidates, manual.' },
          league: { type: 'string', description: 'League for this feedback, e.g. NBA, Tennis, UFC.' },
          notes: { type: 'string', description: 'Optional freeform notes.' }
        },
        required: ['betId', 'skipReason'],
        additionalProperties: false
      }
    },
    {
