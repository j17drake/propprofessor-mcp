 ...
 * legacy key (e.g. `is_live`) while callers can use the cleaner canonical
 * key (e.g. `live`). Applied AFTER validateArgs has accepted the call, so
 * the schema's known-property check has already passed.
 *
 * Currently handles:
 *   - `live` ↔ `is_live` (all tools)
...
function normalizeArgs(toolName, args) {
  ...
  const out = { ...args };
  // live <-> is_live (handler code reads is_live)
  if (out.live === undefined && out.is_live !== undefined) {
    out.live = out.is_live;
  } else if (out.is_live === undefined && out.live !== undefined) {
    out.is_live = out.live;
  }
