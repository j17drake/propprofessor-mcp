...
const WATCHLIST_PATH = process.env.PP_SPORTS_WATCHLIST_PATH
...
function scoreTweet(tweet, sport) {
  ...
  const author = String(tweet.author || '').toLowerCase().replace(/^@/, '');
...
const INJURY_KEYWORDS = /\b(injur(?:y|ed|ies)?|out for|withdrew|withdrawal|surgery|illness|concussion|strain|sprain|tear|broken|fracture|suspended|DNP|day-to-day|ruled out)\b/i;
...
function assessRiskFlag(tweets = [], news = []) {
  ...
  for (const item of allItems) {
    if (item.score >= HIGH_AUTHORITY_THRESHOLD
        && INJURY_KEYWORDS.test(item.text)
        && item.ageMin < (RECENT_WINDOW_MS / 60000)) {
      return { riskFlag: 'high', riskTrigger: `High-authority (${Math.round(item.score)}) + injury: "${item.text.slice(0, 200)}"` };
    }
  }
