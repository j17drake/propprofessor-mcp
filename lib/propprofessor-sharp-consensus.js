 ...
 * @returns {Object} { score, consensusWindowCount, totalWindows, consensusWindows, hasInsufficientData }
 */
function computeMultiWindowScore(row, options = {}) {
  ...
  for (const w of windows) {
    const cutoff = nowMs - (w * 60 * 60 * 1000);
    const directions = [];
    for (const [, points] of bookHistory) {
    ...
    if (directions.length >= 2 && directions.every((d) => d === directions[0])) {
