...
function gradeMovementQuality(item = {}) {
  ...
  const peakAdverseClv = Number(
    item.peakAdverseClvPct ?? item.movementSummary?.peakAdverseClvPct ?? 0
  );
...
function getKaiCall(item = {}) {
  ...
  if (focusMissing) {
    if (grade === 'green' && riskScore <= 3) return 'CONSIDER';
    if (riskScore <= 3) return 'CONSIDER';
...
function getConfidenceTier(item = {}) {
  ...
  // BUGFIX (2026-06-21): kaiCall=BET must never get TIER 4.
  // The risk-score and movement-grade based tier calculation can produce
  // TIER 4 for plays that kaiCall rates as BET (e.g. mixed movement
  // on a 5-book consensus with 1.48% edge and best execution). Since
  // kaiCall is the authoritative actionability signal — it considers
  // more factors than the raw tier function — override TIER 4 back to
  // TIER 3 when kaiCall says BET or CONSIDER.
  // This prevents the contradiction where a play says "BET" but "TIER 4 (avoid)".
  if (call === 'BET' || call === 'CONSIDER') {
    // kaiCall already considered all signals. The tier is used for
    // presentation only — cap the tier at what the call suggests.
    // BET → at least TIER 2, CONSIDER → at least TIER 3.
    const minTier = call === 'BET' ? 'TIER 2' : 'TIER 3';
    // Compute the raw tier without the kaiCall override to see where
    // the risk-score-based tier lands, then clamp upward.
    const rawTier = (() => {
      if (grade === 'green') {
        if (riskScore <= 2) return 'TIER 1';
        if (riskScore <= 4) return 'TIER 2';
        if (riskScore <= 6) return 'TIER 2';
        return 'TIER 3';
      }
      // Yellow
      if (riskScore <= 4) return 'TIER 2';
    ...
    const tierLevels = { 'TIER 1': 1, 'TIER 2': 2, 'TIER 3': 3, 'TIER 4': 4 };
    const rawLevel = tierLevels[rawTier] || 4;
    const minLevel = tierLevels[minTier] || 3;
    // Use whichever is better (lower level number = better tier)
    return Object.keys(tierLevels).find((k) => tierLevels[k] === Math.min(rawLevel, minLevel)) || minTier;
  }
...
function getConfidenceTierStable(item = {}) {
  ...
  // Record in score timeline (persists across requests)
  ...
  const evolvingTier = Object.entries(tierCounts).sort((a, b) => b[1] - a[1])[0][0];
...
function buildRationale(item = {}) {
  ...
  const grade = gradeMovementQuality(item);
  const tier = item.confidenceTier || getConfidenceTierStable(item);
