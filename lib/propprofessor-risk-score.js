...
function gradeMovementQuality(item = {}) {
  ...
  const strongConsensusThreshold = consensusBookCount >= 10;
  ...
  const bouncyStrongConsensus = consensusBookCount >= 12 && Boolean(item.pinnacle);
  ...
  const strongConsensus = league === 'MLB' ? consensusBookCount >= 3 : consensusBookCount >= 5;
...
function getConfidenceTier(item = {}) {
  ...
  if (result.tier === 'TIER 2' && grade === 'yellow' && riskScore <= 3) {
    const edge = Number(item.consensusEdge ?? item.edge ?? 0);
    const disposition = String(item.movementDisposition || '').toLowerCase();
    const TIER1_HIGH_EDGE_THRESHOLD = 2.5;
    // Insufficient history can't confirm a trend — the whole point is CLV
    // by gametime. No trend data = no TIER 1, regardless of edge.
