...
function getConfidenceTierStable(item = {}) {
  ...
  const evolvingTier = Object.entries(tierCounts)
...
function getTierTrajectory(item = {}) {
  ...
  if (entries.length < 2) {
    return { trend: 'new', volatility: 'unknown', dataPoints: entries.length, currentRisk: entries[0]?.riskScore || 0, avgRisk: entries[0]?.riskScore || 0, riskRange: 0 };
  }
