 ...
 */
async function findMlbGamePk({ isoDate, awayTeam, homeTeam } = {}) {
  if (!/^\d{4}-\d{2}-\d{2}$/.test(isoDate || '')) return null;
  if (!awayTeam || !homeTeam) return null;
  const cacheKey = `lookup:${isoDate}:${awayTeam.toLowerCase()}:${homeTeam.toLowerCase()}`;
  const cached = _scheduleCache.get(cacheKey);
  ...
  const result = match ? String(match.gamePk) : null;
