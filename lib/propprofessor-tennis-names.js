...
function resolvePlayerName(name) {
  if (!name) return null;
  const key = name.trim().toLowerCase().replace(/[^\w\s-]/g, '').replace(/\s+/g, '-');
...
function getNameSlug(name) {
  ...
  if (full && full !== 'unknown') {
    return full.toLowerCase().replace(/[^\w\s-]/g, '').replace(/\s+/g, '-');
  }
  // Try direct slug from last name
  const slug = name.trim().toLowerCase().replace(/[^\w-]/g, '').replace(/\s+/g, '-');
  return slug || null;
}
