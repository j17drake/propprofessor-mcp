...
function categorizeError(error) {
  ...
  if (error?.code === 'TOKEN_REFRESH_FAILED_BOTH_PATHS' || lower.includes('token refresh failed both paths')) {
    ...
    return createCategorizedError({
      ...
      status: error?.status,
      cause: error,
      recovery: error?.recovery || (isRateLimited
  ...
  if (lower.includes('no row matched selection')) {
    return createCategorizedError({
      ...
      status: error?.status,
      cause: error,
      recovery: 'The selection name may not match. For handicaps/spreads, try the team or player name without the line (e.g. "Argentina" not "Argentina -1.5"). For totals, strip the "Over "/"Under " prefix. For soccer, the store returns selections as team names only.'
    });
  }

  if ((lower.includes('no candidates') || lower.includes('0 candidates') || lower.includes('empty_payload')) &&
    ...
    return createCategorizedError({
      ...
      status: error?.status,
      cause: error,
      recovery: 'Soccer uses Draw No Bet / Match Handicap / Total Goals — NOT Moneyline / Spread / Total. Call get_market_registry("Soccer") to see available markets.'
    });
  }
  return createCategorizedError({
    message,
